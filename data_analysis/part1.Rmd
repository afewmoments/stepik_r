...
```{r}
drugs <- read.csv("data/lekarstva.csv")
...
```{r}
t <- read.table("data/dataset_11504_15.txt")
...
```{r}
mydata <- read.csv('data/shops.csv')
str(mydata)
```
...
```{r}
mydata2 <- read.csv('data/therapy_data.csv')
str(mydata2)
```
...
```{r}
pills <- read.csv('data/Pillulkin.csv')
...
```{r}
homosc <- read_csv("data/homosc.csv")
...
Логистическая регрессия позволяет исследовать как различные переменные причем как количественные так и номинативные влияеют на зависимую переменную с двумя градациями.

Самой простой вариант --- событие может случится или нет.

$$p_i = \beta_0 + \beta_1 x_{1,i} + \beta_2 x_{2,i} + ... + \beta_k x_{k,i}$$

Что хотим добиться при помощи логистической регрессии?
Мы хотим чтобы в левой части уравнения мы могли рассчитать вероятность того что событие произойдёт.
А в правой есть переменные икс-ы, и какие-то коэффициенты.

В чём проблема? в том что переменные могут принимать значения от - бесконечности до плюс бесконечности.
А вероятность может принимать значения от нуля до 1.

Выход есть. Вот то, что мы хотим получить

$$p_i = \frac{exp(\beta_0 + \beta_1 x_{1,i} + \beta_2 x_{2,i} + ... + \beta_k x_{k,i})}{1+exp(\beta_0 + \beta_1 x_{1,i} + \beta_2 x_{2,i} + ... + \beta_k x_{k,i})}$$

Разберём по шагам.

Основная задача стоит такая --- трансформировать значения вероятности таким образом, чтобы оно принимало значения от - бесконечности до плюс.

НАчнём с того что расчитаем odds 

![Отношение вероятности успеха, к вероятности неудачи](img/odds.png)

Теперь просто возьмём натуральный логарифм от odds

![Отношение вероятности успеха, к вероятности неудачи](img/odds1.png)

И теперь мы готовы посмотреть на итоговую картинку.

![Отношение вероятности успеха, к вероятности неудачи](img/odds2.png)

### Логистическая регрессия практика

рассммотрим искусственный набор данных

```{r}
my_df <- read.csv("data/train.csv", sep=";")
str(my_df)
```

* hon --- переменная получил диплом или не получил.

* math, write, read --- баллы по математике, письму и чтению

* gender --- пол

Мы будем смотреть, как разные переменные пол и различные успеваемости по предметам могут предсказать тот факт, окончит человек с красным дипломом или нет
...
Наблюдается положительная связь между баллами по математике и чтению. На взгляд, кажется что для того чтобы получить красный диплом выгоднее быть девушкой.
...
Теперь мы используем обобщённую линейную модель. Разберём простую модель без взаимодействий. Биноминал --- используем регрессионую модель.

Intercept --- это показатель который будет рассчитан для женщин при условии что у них ноль баллов по чтению и математике. Как это интерпретировать --- если наблюдаемый женщина, у него ноль баллов по математике и чтению, то вероятность получить красный диплом крайне мала. Если посмотреть на 

![Отношение вероятности успеха, к вероятности неудачи](img/odds2.png)

И сравнить с Intercept, видно что -12 очень близко к вероятности ноль

Попробуем теперь проинтерпретировать коэффициенты которые стоят при количественных переменных.

read 0.06677.

По умолчанию в интерцепте принято что испытуемый женщина. Если испытуемый женщина, если балы по математике фиксированы и не изменяются, то с единичным увеличением балов по чтению значение логорифма odds будет увеличиваться приблизительно на 0.06677

math 0.13907

Та же история. Если испытуемый женщина, если балы по математике фиксированы и не изменяются, то с единичным увеличением балов по математике значение логорифма odds будет увеличиваться приблизительно на 0.13907

gendermale -1.18606

Получается как только переходим с градации женщина, на градацию мужчина вероятность получить диплом уменьшится.
...
Запишем переменную в которую запишем предсказанные значения 

```{r}
my_df$prob  <- predict(object = fit, type = "response")
```

Биноминальная регрессия возвращает вероятности. Получили 0.42, это много или мало? 

Рассмотрим базовые методы работы.
...
```{r}
ggplot(data = ToothGrowth, aes(supp, len, fill = factor(dose))) +
  geom_boxplot()
```

### Логистическая регрессия оценка

ROC-кривые. Подгрузить пакет
...
Мы это визуализировали, мы это интерпретировали. Но мы не установили порог остечения.

Для этого посмотрим как между собой соотносится специфичность и чувствительность классификатора.

Рок-кривая показывает соотношение тру-позитив рэйт и фолс-позитив рэйт. Но не указывает какой порог нужно ставить.

Стандартное решение посмотреть как соотносится специфичность и чувствительность классификатора
...
Специфичность --- на сколько хорошо удаётся правильно предсказывать отрицательный исход события.

Логично взять порогом отсечения точку пересечения трёх показателей

Примем основное решение --- как будем классифицировать наших испытуемых.

Добавим переменную, это будет не просто вероятность, но конкретное значение, либо получил либо не получил диплом.

И добавим вторую переменную, которая будет сравнивать два значения, и говорить правильно ли мы предсказали или нет
...
Построим небольшой график, который будет показывать на сколько хорошо наша модель предсказала
...
Построим эффективность нашей модели

```{r}
mean(my_df$correct)
```

Ну и разумеется, мы можем использовать полученные значения, для того чтобы предсказывать на новых данных. 

```{r}
test_df  <- read.csv("data/test.csv", sep = ";")
test_df$hon  <- NA

# У меня не сработало, но код должен быть такой
...
```{r}
# Сначала разобъём данные на две группы, те на которых будем обучаться, и те которые будем предсказывать
data_na <- subset(data, is.na(data$admit) == TRUE)
data_clean <- subset(data, is.na(data$admit) == FALSE)

# Построим модель, с учётом взаимодействия двух переменных
fit_data <- glm(admit ~ rank * gpa, data_clean, family = "binomial")

# Используем полученную модель, для предсказания пропущенных значений
data_na$admit  <- predict(fit_data, newdata = data_na, type = "response")

# Введём новую номинативную переменную, которая в зависимости от указанного порога говорит поступил чел, или нет
data_na$pred_resp  <- factor(ifelse(data_na$admit > 0.4, 1, 0), labels = c("N", "Y"))

# Считаем количество поступивших
sum(data_na$pred_resp == "Y")
```